<?php
    ...
    if($conn->connect_error){
        die('Connect Error(' . $conn->connect_error . ') ' .$conn->connect_error);
            ...
            $stmt=$conn->prepare("INSERT INTO contect(name,email,message) VALUES(?,?,?)");
        if($stmt===false){
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Your message was successfully sent!'); window.location.href = 'Contect.php';</script>";
        exit();
    } else {
        echo "<script>alert('There was an error: " . $stmt->error . "'); window.history.back();</script>";
        exit();
    }
        $stmt->close();
        $conn->close();
...
<div class="BODY">
    <div class="form-container">
        <form action="messsage.php" method="POST">
            <div class="mb-3">
