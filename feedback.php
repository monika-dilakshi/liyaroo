<?php
    ...
    if($conn->connect_error){
        die('Connect Error(' . $conn->connect_error . ') ' .$conn->connect_error);
            ...
            $stmt=$conn->prepare("INSERT INTO review(name,email,message) VALUES(?,?,?)");
        if($stmt===false){
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Your message was successfully sent!'); window.location.href = 'review.php';</script>";
        exit();
    } else {
